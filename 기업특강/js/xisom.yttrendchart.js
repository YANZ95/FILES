...
(function () {
    function YTTrendChart(arg) {
        ...
        for (var i = 0; i < arg.series.length; i++) {
            if (arg.series[i].tag.indexOf("_Params") == 0) {
                arg.series[i].tag = $XT(arg.series[i].tag).name;
            }
        }
        ...
        this.viewmodeInternal = arg.viewmode;
        this.samplingmodeInternal = arg.samplingmode;
        this.timeSpan = arg.trend.timeSpan;
        ...
        this.LiveTimeoutID = [];
        // TimeSpan variables 
        this.viewmodeList = arg.filebuffer.samplingoptions;
        // SamplingList
        this.samplingList = arg.filebuffer.samplingtimes;
        // List of TimeSpan
        this.timespanList = arg.filebuffer.timespans;
        this.timespanIdx = this.getTimeSpanIdx(this.trend.timeSpan);
        this.timespanIdMin = 0;
        this.timespanIdMax = this.timespanList.length - 1;
    }
    ...
    YTTrendChart.prototype.setBorderStyle = function (input) {
        ...
        $("#" + this._id + "-view").css("border-style", value);
        this.ytappearance.borderStyle = input;

        var bordercolor = this.ytappearance.borderColor;
        $("#" + this._id + "-view").css("border-color", this.optionColor(bordercolor));

        var borderwidth = this.ytappearance.borderWidth;
        $("#" + this._id + "-view").css("border-width", borderwidth);
    }
    ...
    YTTrendChart.prototype.getRefreshTime = function () {
        if (this.viewmodeInternal == "realtime")
    ...
    YTTrendChart.prototype.getOriginalTimeSpan = function () {
        return this.trend.timeSpan;
        //return TimeSpan;
    }
    YTTrendChart.prototype.getTimeSpanIdx = function (value) {
        switch (this.viewmodeInternal) {
            case "realtime": {
                this.setTimeSpan(this.timespanList[0]);
                return 0;
            }
            case "daily": {
                this.setTimeSpan(this.timespanList[1]);
                return 1;
            }
            case "weekly": {
                this.setTimeSpan(this.timespanList[2]);
                return 2;
            }
            case "monthly": {
                this.setTimeSpan(this.timespanList[3]);
                return 3;
            }
            case "quarterly": {
                this.setTimeSpan(this.timespanList[4]);
                return 4;
            }
            case "yearly": {
                this.setTimeSpan(this.timespanList[5]);
                return 5;
            }
        }
    }
    ...
    YTTrendChart.prototype.setStartTimeWithValue = function (value) {
        // console.log("setStartTime")
    ...
    YTTrendChart.prototype.optionSeriesByNameMode = function (mode, name) {
        var oSeries = this.getSeries();
        var samplingmode = this.samplingmodeInternal;

        var seriesCount = oSeries.length;
        for (var i = 0; i < seriesCount; i++) {
            var s = oSeries[i];
            if (!s) continue;
            if (s.tag == name && (s.samplingmode == mode || samplingmode == mode.toLowerCase())) return s;
        }
        return null;
    }
    ...
    YTTrendChart.prototype.optionXaxis = function () {
        ...
        var starttime = new Date(endtime.getTime() - timespan * 1000);
        ...
        var item = {
            ...
            axisLabel: {
                ...
                formatter: function (value, index) {
                    ...
                    return $.format.date(value, view.ytxaxis.format.format);
                },
    ...
    YTTrendChart.prototype.optionToolTip = function () {
        ...
        */
        var item = {
            trigger: 'axis',
            formatter: function (params, ticket, Display) {
                var retData = "";
                retData += "<table>";
                var length = params.length;
                if (length == 0) return;
                var title = "<h4 style=\"text-align:center;\">" + new Date(params[0]['axisValue']).format("yyyy-MM-dd HH:mm:ss") + "</h4>";
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    retData += "<tr>";
                    retData += "<th>";
                    retData += item['marker'] + item['seriesName'];
                    retData += "</th>";
                    retData += "<td>";
                    retData += "</td>";
                    retData += "<td style=\"padding-left: 10px;\">";
                    retData += Math.round(item['data'][i + 1] * 100) / 100;
                    retData += "</td>";
                    retData += "</tr>";
                }
                retData += "</table>";
                var ret = title + retData;
                return ret;
            }
        }
        return item;
    ...
    YTTrendChart.prototype.toolbarButtonID = function (type) {
        ...
        else if (type == "viewmodelabel") return this._id + "-viewmodelabel";
        else if (type == "samplingmode") return this._id + "-samplingmode";
        else return "";
    }
    ...
    YTTrendChart.prototype.initToolbar = function () {
        ...
        var viewmodelabel = this.toolbarButtonID("viewmodelabel");
        var samplingmode = this.toolbarButtonID("samplingmode");

        // Only show viewmode in sampling option of filebuffer
        var viewmodeHtml = "";
        if (this.viewmodeList.length > 0) {
            if (this.viewmodeList.indexOf('realtime') > -1)
                viewmodeHtml += '<option value="600">RealTime</option>';
            if (this.viewmodeList.indexOf('hourly1h') > -1)
                viewmodeHtml += '<option value="3600">Hourly</option>';
            if (this.viewmodeList.indexOf('hourly2h') > -1)
                viewmodeHtml += '<option value="7200">Hourly 2H</option>';
            if (this.viewmodeList.indexOf('hourly6h') > -1)
                viewmodeHtml += '<option value="21600">Hourly 6H</option>';
            if (this.viewmodeList.indexOf('daily') > -1)
                viewmodeHtml += '<option value="86400">Daily</option>';
            if (this.viewmodeList.indexOf('weekly') > -1)
                viewmodeHtml += '<option value="604800">Weekly</option>';
            if (this.viewmodeList.indexOf('monthly') > -1)
                viewmodeHtml += '<option value="2592000">Monthly</option>';
            if (this.viewmodeList.indexOf('quarterly') > -1)
                viewmodeHtml += '<option value="7776000">Quarterly</option>';
            if (this.viewmodeList.indexOf('yearly') > -1)
                viewmodeHtml += '<option value="31536000">Yearly</option>';
        }
        ...
        buttonHtml += '<a><select name="viewmodelabel" id="' + viewmodelabel + '" >' + viewmodeHtml + '</select></a> ';
        buttonHtml += '<a><select name="samplingmode" id="' + samplingmode + '" ><option value="maximum">Max</option><option value="average">Avg</option><option value="minimum">Min</option></select></a>';
        ...
        $("#" + viewmodelabel).change(function () {
            view.ViewModeCBChangeHandler();
        });
        $("#" + samplingmode).change(function () {
            view.SamplingModeChangeHandler();
        });
    ...
    // Setup ViewMode Label
    YTTrendChart.prototype.toolbarBySetViewModeCB = function (idx) {
        var view = this;
        var viewmodeLable = view.toolbarButtonID("viewmodelabel");
        var tslabel = document.getElementById(viewmodeLable);

        if (tslabel == null) return;

        // change viewmode
        this.viewmodeInternal = this.viewmodeList[idx];

        // Set ViewMode
        tslabel.selectedIndex = idx;

        if (idx == 0)
            view.DisableCBSamplingMode(true);
        else
            view.DisableCBSamplingMode(false);
    }

    // Handle TimeSpanCB Change change in case of normal
    YTTrendChart.prototype.ViewModeCBChangeHandler = function () {
        var view = this;
        var viewmodelabel = view.toolbarButtonID("viewmodelabel");
        var element = document.getElementById(viewmodelabel);

        var idx = element.selectedIndex;
        var timespan = view.timespanList[idx];
        view.timespanIdx = idx;
        if (idx == 0) {
            view.DisableCBSamplingMode(true);
        }
        else {
    ...
    // Handle Sampling Mode change in case of normal
    YTTrendChart.prototype.SamplingModeChangeHandler = function () {
        var samplingmode = this.toolbarButtonID("samplingmode");
        var smElement = document.getElementById(samplingmode);

        //maximum
        if (smElement.selectedIndex == 0) {
            this.samplingmodeInternal = "maximum";
        }
        // average
        else if (smElement.selectedIndex == 1) {
            this.samplingmodeInternal = "average";
        }
        // minimum
        else if (smElement.selectedIndex == 2) {
            this.samplingmodeInternal = "minimum";
        }
    }

    YTTrendChart.prototype.ChangeCBSamplingMode = function (value) {
        var samplingmode = this.toolbarButtonID("samplingmode");
        var smElement = document.getElementById(samplingmode);
        ...
        if (value == "maximum") {
            this.samplingmodeInternal = "maximum";
            smElement.selectedIndex = 0;
        } else if (value == "average") {
            this.samplingmodeInternal = "average";
            smElement.selectedIndex = 1;
        } else if (value == "minimum") {
            this.samplingmodeInternal = "minimum";
            smElement.selectedIndex = 2;
        }
    }
    YTTrendChart.prototype.DisableCBSamplingMode = function (state) {
        var samplingmode = this.toolbarButtonID("samplingmode");
        var smElement = document.getElementById(samplingmode);

        if (state == true) { // => Disable        
            smElement.setAttribute("disabled", "disabled");
        } else { // => Enable
            smElement.removeAttribute("disabled");
        }
    }
    ...
    YTTrendChart.prototype.toolbarByXZoomIn = function () {
        ...
        var view = this;
        if (view.timespanIdx == view.timespanIdMin) return;

        var grid = view.getGrid();
        var majorCount = grid.vcount;
        var timespan = this.getOriginalTimeSpan();

        // Update TimeSpan Label
        view.toolbarBySetViewModeCB(--this.timespanIdx);

        // realtime mode
        var newtimespan = 0;
        if (this.timespanIdx == 0) {
            newtimespan = Math.ceil(timespan / majorCount) * majorCount;

        } else {
            newtimespan = Math.ceil(this.timespanList[this.timespanIdx] / majorCount) * majorCount;
        }
    ...
    YTTrendChart.prototype.toolbarByXZoomOut = function () {
        ...
        var view = this;
        if (view.timespanIdx == view.timespanIdMax) return;
        ...
        var newtimespan = Math.ceil(this.timespanList[++this.timespanIdx] / majorCount) * majorCount;
        ...
        $("#" + contentID).dialog({
            ...
            buttons: {
                " Apply ": function (evt) {
                    ...
                    if (view.viewmodeInternal == 'realtime') {
                        timeSpan = view.getOriginalTimeSpan();
                    } else {
                        timeSpan = view.timespanList[view.timespanIdx];
                    }
                    endTime = new Date(st.setSeconds(st.getSeconds() + timeSpan))
    ...
    YTTrendChart.prototype.seriesDataByValue = function (yValue) {
        ...
        for (var i = 0; i < seriesCount; i++) {
            ...
            var name = this.optionSeriesName(s);
            var axisindex = 0;
            if (s.axisExtra == "Y2_Axis") {
                axisindex = 1;
            }
            else if (s.axisExtra == "Y3_Axis") {
                axisindex = 2;
            }
            else if (s.axisExtra == "Y4_Axis") {
                axisindex = 3;
            }
            var item = {
    ...
    YTTrendChart.prototype.drawChart = function () {
        ...
        if (timespan <= 10) {
            ...
            var timespan = this.getTimeSpan();
            var split = Math.round(timespan / majorCount);
            // optionXaxis.axisLabel.interval = 60000;
            optionXaxis.axisLabel.interval = split;
        }
    ...
    YTTrendChart.prototype.setValueByViewData = function (data) {
        ...
        for (var k = 1; k < data.names.length; k++) {
            ...
            if (this.viewmodeInternal == "normal") {
                s = this.optionSeriesByName(name);
            }
            else {
                ...
                s = this.optionSeriesByNameMode(mode, parts[1]);
            }
            if (s == null) continue;
            //["Time", "Data A", "Data B"]
            var t = this.optionSeriesName(s);
    ...
    YTTrendChart.prototype.doViewData = function () {
        ...
        if (isLive) {
            // Clear LiveTimeoutID
            var length = this.LiveTimeoutID.length;
            for (var i = 0; i < length; i++) {
                var id = this.LiveTimeoutID.pop();
                clearTimeout(id);
            }
    ...
    YTTrendChart.prototype.doViewDataByLive = function () {
        ...
        for (var i = 0; i < oSeries.length; i++) {
            ...
            var tag = $XT(element.tag);
            if (tag == null) continue;

            if (element.samplingmode == "Average")
                oTags.push("Avg " + tag.name);
            else if (element.samplingmode == "Maximum")
                oTags.push("Max " + tag.name);
            else if (element.samplingmode == "Minimum")
                oTags.push("Min " + tag.name);
            else {
                if (view.samplingmodeInternal == "maximum")
                    oTags.push("Max " + tag.name);
                else if (view.samplingmodeInternal == "average")
                    oTags.push("Avg " + tag.name);
                else if (view.samplingmodeInternal == "minimum")
                    oTags.push("Min " + tag.name);
            }
        }
        ...
        params = {
            mode: "liveFB",
            viewmode: view.viewmodeInternal,
            stype: view.samplingmodeInternal,
            timeStamp: endtime,
            timeSpan: timeSpan,
            tags: oTags
        }
        ...
        scada.getViewLiveDataYTChart(view.pid, view.id, params, function (data) {
            ...
            view.LiveTimeoutID.push(timeoutID);
        }, limitPoint);
    }

    // Add dummy data which selected from file buffer
    YTTrendChart.prototype.addDummyDataLive = function (data, endtime) {
        ...
        var partials = Math.ceil(datalength / majorCount);
        var newLength = partials * majorCount;
        var addLength = partials * majorCount - datalength;

        // Add dummy package to starttime
        if (starttime.getTime() < firsttime.getTime()) {
            var val = []
            for (var i = 1; i < namelength; i++) {
                val.push(NaN);
            }
            val.unshift(starttime);
            data.items.unshift(val);
            addLength--;
        }

        // Add dummy package to endtime
        if (endtime.getTime() > lasttime.getTime()) {
            var val = []
            for (var i = 1; i < namelength; i++) {
                val.push(NaN);
            }
            for (var i = addLength; i > 0; i--) {
                var pusVal = val.slice(0, val.length - 1);
                pusVal.unshift(new Date(endtime.getTime() - i));
                data.items.push(pusVal);
            }
        }

        //Check and replace NaN for first and last value
        var _firstData = data.items[0];
        var firstData = []
        _firstData.forEach(element => {
            if (element == "NaN") {
                firstData.push(NaN);
            }
            else {
        ...
        data.items[0] = firstData;

        var _lastData = data.items[datalength - 1];
        var lastData = [];
        _lastData.forEach(element => {
            if (element == "NaN") {
                lastData.push(NaN);
            } else {
        ...
        data.items[datalength - 1] = lastData;
    }

    YTTrendChart.prototype.addDummyDataTrend = function (data) {
        // console.log("addDummyDataTrend");
        var view = this;
        var datalength = data.items.length;
        if (datalength <= 0) return;
        // var starttime = this.getStartTime();
        ...
        if (endtime.getTime() > lasttime.getTime()) {
            ...
            // val.unshift(endtime.format("YYYY-MM-DDTHH:mm:ss.SSSZ"));
            // data.items.push(val);
            for (var i = addLength; i > 0; i--) {
                var pusVal = val.slice(0, val.length - 1);
                pusVal.unshift(new Date(endtime.getTime() - i));
    ...
    YTTrendChart.prototype.ImportSettingOperation = function (contents, view) {
        ...
        view.importYTTrendChartState({
            "@ViewMode": setting.getAttribute("ViewMode"),
            "@SpanTimeIndex": setting.getAttribute("SpanTimeIndex"),
            "@SamplingMode": setting.getAttribute("SamplingMode")
        });
        ...
        view.importXAxis({
            ...
            "@LabelAngle": xaxis.getAttribute("LabelAngle"),
            "@LabelFormatter": xaxis.getAttribute("LabelFormatter"),
        ...
        for (var i = 0; i < childLength; i++) {
            ...
            var item = {
                ...
                "@Text": seriesItem.getAttribute("Text"),
                ...
                "@SamplingMode": seriesItem.getAttribute("SamplingMode"),
    ...
    YTTrendChart.prototype.importSettingOperation = function (name) {
        ...
        scada.getYTTrendChartXML(name, function (data) {
            ...
            view.importYTTrendChartState(setting);
    ...
    YTTrendChart.prototype.importYTTrendChartState = function (setting) {
        var view = this;
        view.viewmodeInternal = setting['@ViewMode'];

        var idx = view.viewmodeList.indexOf(view.viewmodeInternal);

        view.toolbarBySetViewModeCB(idx);
        view.timespanIdx = idx;

        view.samplingmodeInternal = setting['@SamplingMode'];

        view.ChangeCBSamplingMode(view.samplingmodeInternal);
        ...
        var majorCount = grid.vcount;
        var timelimit = view.getLimitTime();
        var newtimespan = Math.ceil(view.timespanList[idx] / majorCount) * majorCount;
    ...
    YTTrendChart.prototype.importXAxis = function (setting) {
        ...
        view.ytxaxis.label.angle = setting["@LabelAngle"];
        view.ytxaxis.isReversed = setting["@IsReversed"] == "true" ? true : false;
        view.ytxaxis.format.format = setting["@LabelFormatter"];
    ...
    YTTrendChart.prototype.exportSettingOperation = function (name) {
        ...
        function exportAppearance() {
            ...
            appearance += ' BorderStyle=' + quotes + ((config.borderStyle === "solid") ? "Single" : "None") + quotes;
            appearance += ' />';
            return appearance;
        }
        function exportGrid() {
        ...
        function exportXAxis() {
            ...
            ret += ' LabelAngle=' + quotes + config.label.angle + quotes;
            ret += ' IsReversed=' + quotes + config.isReversed + quotes;
            ret += ' LabelFormatter=' + quotes + config.format.format + quotes;
            ret += ' />';
            return ret;
        }
        ...
        function exportSeries() {
            ...
            function SeriesFormat(config) {
                ...
                item += ' Text=' + quotes + config.text + quotes;
                ...
                item += ' SamplingMode=' + quotes + config.samplingmode + quotes;
        ...
        setting += '<yTTrendChart ViewMode="' + view.viewmodeInternal + '" SpanTimeIndex="' + view.timespanIdx + '" SamplingMode="' + view.samplingmodeInternal + '">' + newline;
    ...
    YTTrendChart.prototype.EditSeriesOperation = function () {
        ...
        if (!listCreated) {
            ...
            pgMetaObj = {
                ...
                text: { group: 'Properties', name: 'Text' },
                tag: { group: 'Properties', name: 'Tag', type: 'options', options: listtag, default: "None" },
                samplingmode: { group: 'Properties', name: 'SamplingMode', type: 'options', options: ['None', 'Maximum', 'Average', 'Minimum'], disable: false },
                axis: { group: 'Properties', name: 'AxisExtra', type: 'options', options: ['Y1_Axis', 'Y2_Axis', 'Y3_Axis', 'Y4_Axis'] },
    ...
    YTTrendChart.prototype.AddSeriesButtonClick = function () {
        ...
        var seriestext = view.GenSeriesText();
        ...
        var seriesitem = { color: seriescolor, label: { font: { name: "Tahoma", size: 10, bold: false, italic: false, underline: false }, position: "Default", showPercent: false, showValue: false, textColor: "#000000" }, line: { lineColor: "", lineStyle: "Solid", lineWidth: 1, visible: true }, point: { color: "", size: 5, visible: false, type: "Circle" }, opacity: 1, tag: inittag, axisExtra: "YAxis", name: seriesname, text: seriestext, visible: true, type: "Line" };
    ...
    // Generate a Text for series
    YTTrendChart.prototype.GenSeriesText = function () {
        var seriesitems = document.getElementsByClassName("seriesitem");
        var i = 0;
        var name = "";
        for (; ;) {
    ...
    YTTrendChart.prototype.UpdatePropertiesGrid = function () {
        ...
        var setObj = {
            ...
            samplingmode: currentseries.samplingmode,
    ...
    YTTrendChart.prototype.propertyChangedCallback = function (grid, name, value) {
        ...
        switch (name) {
            ...
            case "samplingmode": {
                currentseries.samplingmode = value;
                break;
            }
        ...
        if (!isGeneralSettingInit) {
            ...
            gsMetaObj = {
                ...
                XAxis: {
                    ...
                    labelAngle: { group: 'XAxis', name: 'Label Angle', type: 'input' },
                    labelFormatter: { group: 'XAxis', name: 'Label Formatter', type: 'input' },
    ...
    YTTrendChart.prototype.GetChartProperties = function () {
        ...
        gsXaxisProperties = {
            ...
            labelAngle: xaxis.label.angle,
            labelFormatter: xaxis.format.format,
    ...
    YTTrendChart.prototype.GSXaxisChangedCallback = function (grid, name, value) {
        ...
        if (name == 'name') {
            ...
            gsXaxisProperties.labelAngle = value;
        } else if (name == 'labelFormatter') {
            gsXaxisProperties.labelFormatter = value;
    ...
    YTTrendChart.prototype.GSApplySetting = function () {
        ...
        xaxis.label.angle = gsXaxisProperties.labelAngle;
        xaxis.format.format = gsXaxisProperties.labelFormatter;
    ...
    YTTrendChart.prototype.initChart = function () {
        ...
        // Setup ViewMode
        this.ChangeCBSamplingMode(this.samplingmodeInternal);

        // Init TimeSpan label
        this.toolbarBySetViewModeCB(this.timespanIdx);
    ...
    Object.defineProperty(YTTrendChart.prototype, "viewmode", {
        get: function () {
            return this.viewmodeInternal;
        },
        set: function (value) {
            var view = this;
            var idx = view.viewmodeList.indexOf(value);
            if (idx >= 0) {
                var grid = view.getGrid();
                var majorCount = grid.vcount;
                var timespan = this.getOriginalTimeSpan();
                ...
                // realtime mode
                var newtimespan = 0;
                if (view.timespanIdx == 0) {
                    newtimespan = Math.ceil(timespan / majorCount) * majorCount;

                } else {
                    newtimespan = Math.ceil(view.timespanList[view.timespanIdx] / majorCount) * majorCount;
                }
    ...
    Object.defineProperty(YTTrendChart.prototype, "samplingmode", {
        get: function () {
            return this.samplingmodeInternal;
        },
        set: function (value) {
            this.ChangeCBSamplingMode(value);

            // Upate data
            this.doViewData();
        }
    });
    Object.defineProperty(YTTrendChart.prototype, "isLive", {
        get: function () {
    ...
    YTTrendChart.prototype.timeSelection = function (starttime) {
        var islive = this.getIsLive();
        if (islive == true) return;
        if (starttime == null) {
            ...
            var endtime = new Date(_starttime.setSeconds(_starttime.getSeconds() + this.getTimeSpan()));
    ...
    function ChartSeries(owner, arg) {
        ...
        Object.defineProperty(ChartSeries.prototype, "samplingmode", {
            get: function () {
                var item = this.owner.ytseries[this.index];
                return item.type;
            },
    ...
    function ChartSeriesCollection(owner, arg) {
        ...
        ChartSeriesCollection.prototype.add = function (seriesname, text, seriescolor, seriestag, smode, serieswidth, seriesvisible, axisextra, seriestype, markerstyle, markersize) {
            ...
            var smTmp = smode.toLowerCase();
            if (smTmp == "none") {
                smode = 'None';
            } else if (smTmp == "maximum") {
                smode = 'Maximum';
            } else if (smTmp == "average") {
                smode = 'Average';
            } else if (smTmp == "minimum") {
                smode = 'Minimum';
            }
    ...
    // Get Next Time
    // Period is total second of a period    
    YTTrendChart.prototype.GetNextTime = function (period) {
        var TotalMilliseconds = 86400 * 1000;
        var PeriosMillisecond = period * 1000;
        var partial = TotalMilliseconds / PeriosMillisecond;
        var timenow = new Date();

        var currentMillisecond = timenow.getTime();
        console.log("currentMillisecond: " + currentMillisecond);

        var interval = Math.ceil((partial + 1) * PeriosMillisecond - currentMillisecond);

        return new Date(currentMillisecond + interval);
    }
