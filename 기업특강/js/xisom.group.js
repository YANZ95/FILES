...
(function () {
    ...
    GroupView.prototype.setVisibleByGroup = function (id, parentVisible) {
        ...
        var currentVisible = view.parentVisible == false ? false : view.visible;
        $("#" + id).attr("visibility", currentVisible == true ? "visible" : "hidden");
		for (var i = 0; i < view.views.length; i++) {
            var nonGroupView = page.getViewByName(view.views[i].id);
            if(nonGroupView.views != null){
                view.setVisibleByGroup(nonGroupView.id, currentVisible);
			} else {
				$XV(view.views[i].id).setVisibleByGroup(currentVisible);
            }			
		}
    }
