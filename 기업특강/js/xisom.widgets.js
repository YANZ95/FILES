...
(function () {
	function ListView(arg) {
		...
		this.scrollX = arg.listView.scrollX;
		this.scrollY = arg.listView.scrollY;
	}
	...
	ListView.prototype.setScrollX = function (value) {
		var body = $("#listview_1").parent().parent()[0];
		if (value == true) {
			this.scrollX = true;
	...
	ListView.prototype.setScrollY = function (value) {
		var body = $("#listview_1").parent().parent()[0];
		if (value == true) {
			this.scrollY = true;
	...
	ListView.prototype.getItemIndexAt = function (pageX, pageY) {
		...
		var contentsY = $('.contents').position().top / scale;
		var headerHeight = this.listView.headerVisible ? listView.headerHeight + 2 : 1; //경계선 보정
		var rowHeight = listView.rowHeight; 

		var y = pageY + $("#" + this._id).closest(".ui-jqgrid-bdiv").scrollTop() - (this.y + headerHeight) - contentsY;
		var maxY = this.items.count * rowHeight;

		if (y <= 0 + 1) return -1;
		if (y >= maxY - 1) return -1;
		if (y % rowHeight <= 2) return -1; // 경계선
...
(function () {
	...
	BarArtView.prototype = Object.create(ShapeElement.prototype);
