...
(function () {
    ...
    UserMenuItemLabel.prototype.getTextFormat = function () {
        return this.textFormat;
    }
    
    UserMenuItemLabel.prototype.setText = function (text) {
        if (text == null) {
        ...
        $("#" + this.id + " span").text(text);
    }
    
    UserMenuItemLabel.prototype.tagValueChanged = function (e) {
    ...
    UserMenuItemLabel.prototype.tagStatusChanged = function (e) {
        if (!this.nullText) return;
    ...
    page.createLabelMenu = function (arg) {
        ...
        if (arg.tag != "") {
            ...
            userMenuItem.tag = tag;
            tag.addEventListener("change", function (event) {
                userMenuItem.tagValueChanged(event);
            });
            if (typeof (arg.nullText) == "string") {
                tag.addEventListener("status", function (event) {
                    userMenuItem.tagStatusChanged(event);
                });
            }
            if(tag.type == "digital"){
                initText = (tag.value) ? tag.onText.toString() : tag.offText.toString();
            } else {
                initText = (tag.value != null) ? tag.value.toString() : userMenuItem.nullText;
            }

            var textFormat = userMenuItem.getTextFormat();
            initText = page.getValueByFormat(tag, textFormat, initText);
        }
        userMenuItem.setText(initText);        
        page.userMenuItems.push(userMenuItem);
...
(function () {
    ...
    UserMenuItemLogin.prototype.refreshMenuItem = function (loginUserInfo) {
        ...
        if(loginUserInfo != null){
            ...
            $("#" + this.id + " span").text(logoutText);
            $("#" + this.id).attr("status", "login");

        }else{
            $("#" + this.id + " span").text(this.text);
