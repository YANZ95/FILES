...
(function () {
	...
	ImageView.prototype.setXlinkHref = function (value, type) {
		if(type == "bytes"){
			if(this.customUrl == "/" + value) return;
	...
	ImageView.prototype.urlTagValueChanged = function (e) {
		...
		if(e.sender.type == "bytes"){
			if(value == "byte[0]"){
				this.setBaseHref();
			} else {
				this.setXlinkHref(e.target.name + "?lastModify=" + e.target.lastModify + "&mimeType=image" + "&animated=" + this.getAnimated(), e.sender.type);
			}
		}else{
			if(value.length == 0){
				this.setBaseHref();
			} else {
				this.setXlinkHref(value, e.sender.type);
			}
		}
	}
