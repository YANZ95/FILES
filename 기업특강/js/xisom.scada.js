...
var scada = (function () {
	...
	var close = function () {
        ...
        if (lang == "ko") {
            ...
            message = "Bạn có chắc bạn muốn thoát khỏi?";
        }
		...
		$.ajax({
			...
			success: function (res) {
				...
				scada.deleteLoginTimeoutCookie();
				//닫기 창으로 이동
				window.location.href = "/_close.html";
			},
			error: function () {
				//브라우져 로그인 관련 쿠키 지우고
				scada.deleteLoginSessionIdCookie();
	...
	var utils = (function () {
		...
		var webRequest = function (url, callback) {
			$.ajax({
		...
		var webRequestPost = function (url, _data, callback, _contentType) {
		...
		var write = function (path, data) {
			var filename;
			if (path == null || path == "") {
				filename = window.prompt("Please enter the file name.");
			} else {
		...
		var readAsDataURL = function (path, callBack, filter) {
			var input = document.createElement("input");
			input.type = "file";
			input.accept = filter.replace(/\*/g, "");
			input.onchange = function (event) {
				processFile(event.target.files[0], callBack);
			};
			input.click();
		}
...
(function () {
	...
	//기간이 만료된 data인지 확인한다.
	Tag.prototype.isExpired = function (data) {
		if (this.lastModify == null || this.lastModify == "") return false;

		var oldLastModifyTime = Date.parse(this.lastModify);
		var newLastModifyTime = Date.parse(data.lastModify);
		return oldLastModifyTime > newLastModifyTime;
	};
	...
	Tag.prototype.putValue = function (data) {
        ...
        var oldLastModify = this.lastModify;
		var tag = this;
		this.status = status;
		this._value = data.value;
		this.datetime = nowDate;

		if (oldLastModify != data.lastModify || oldValue != data.value) {
	...
	Tag.prototype.setValue = function (value, securitykey) {
		...
		$.ajax({
			...
			success: function (res) {
				if (res.code == 200) {
					this.setValued = true;
					scada.tagManager.requestTagData(this);
				} else {
	...
	TagManager.prototype.getTagByName = function (name) {
		if (name.indexOf("_Params.") == 0) {
			var no = name.split(".")[1];
			var index = Number(no) - 1;
			name = page.args[index].name;
		}
		var tags = this.getTags();
	...
	//Get Tags에 대한 tag값 기존
	TagManager.prototype.requestTagData = function (tag) {
		if (scada.getSessionStatus() != 'good') return;
		
		if(this.requestBuffer.findIndex(t=>t.name == tag.name) == -1){
			this.requestBuffer.push(tag);			
		}
		
		if (this.requestReady == false) {
			return;
		}

		this.requestReady = false;
		setTimeout(function () {
			if (scada.tagManager.requestBuffer.length == 1) {
				var tag = scada.tagManager.requestBuffer[0];
				scada.tagManager.requestBufferr = [];
				$.ajax({
					url: '/tag/getValue.xsm',
					type: 'POST',
					dataType: 'JSON',
				...
				var tags = scada.tagManager.requestBuffer;
				var names = scada.tagManager.getTagNames(tags);
				scada.tagManager.requestBuffer = [];
				$.ajax({
					url: '/tag/getValues.xsm',
					type: 'POST',
					dataType: 'JSON',
			...
			scada.tagManager.requestReady = true;
		}, 50);
	}
	...
	// 
	TagManager.prototype.getTagValueString = function (tooltipText) {
		var tags = this.tags;
		var tagDic = new $HashMap();
		var tooltipLine = tooltipText.split('<br/>');
		for (var i = 0; i < tooltipLine.length; i++) {
			var remainLen = tooltipLine[i].length;
			var startidx = 0;
			while (remainLen >= 4) {
				startidx = tooltipLine[i].indexOf("#{", startidx);
				if (startidx >= 0) {
					var tagNameStartIndex = startidx + 2;
					var tagNameEndIndex = tooltipLine[i].indexOf("}", tagNameStartIndex);
					var tagname = tooltipLine[i].substr(tagNameStartIndex, tagNameEndIndex - tagNameStartIndex).trim();
					startidx = tagNameEndIndex + 1;
					remainLen = tooltipLine[i].length - startidx;

					if (tagDic.indexOf("#{" + tagname + "}") == null || tagDic.indexOf("#{" + tagname + "}") == -1) {
						var tag;
						if(tagname.indexOf(":") != -1){
							tag = this.getTagByName(tagname.substr(0, tagname.indexOf(":")));
						}else{
							tag = this.getTagByName(tagname);
						}
						tagDic.put("#{" + tagname + "}", tag);
					}
				} else {
					break;
				}
		...
		var keys = tagDic.keys();
		for (var i = 0; i < keys.length ; i++) {
			var format;
			var tagName;
			if(keys[i].indexOf(":") != -1){
				var tn = keys[i];
				format = $Common.getTextFormatFromText(tn.substr(tn.indexOf(":") + 1).replace("}",""));
				tagName = keys[i].substr(0, tn.indexOf(":"));
			}else{
				tagName = keys[i];
			}
			var regexAllCase = new RegExp(keys[i], "g")
			if(format != null){
				var formattedValue = page.getValueByFormat(tagDic.get(keys[i]), format, tagDic.get(keys[i]).value.toString());
				tooltipText = tooltipText.replace(regexAllCase,formattedValue);
			}else{
				tooltipText = tooltipText.replace(regexAllCase,tagDic.get(keys[i]).value);
			}
		}
...
(function () {
	...
	ViewManager.prototype.getViewLiveData = function (pageName, viewName, params, callback, limit) {
		...
		var tags = params.tags == null ? "" : params.tags.toString();
		$.ajax({
			url: '/view/getData.xsm',
			type: 'POST',
			dataType: 'JSON',
			data: { 'page': pageName, 'view': viewName, 'mode': 'live', 'span': timeSpan, 'limit': limit, 'tags': tags },
			async: true,
			context: this,
			crossDomain: true,
			success: function (res) {
	...
	ViewManager.prototype.getViewLiveDataYTChart = function (pageName, viewName, params, callback, limit) {
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'JSON',
			data: { 'page': pageName, 'view': viewName, 'mode': mode, 'viewmode': viewmode, 'stype': stype, 'tags': tags, 'span': timeSpan, 'limit': limit },
			async: true,
			context: this,
			crossDomain: true,
			success: function (res) {
	...
	ViewManager.prototype.getViewYTTrendData = function (pageName, viewName, params, callback, limit) {
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'JSON',
			data: { 'page': pageName, 'view': viewName, 'mode': mode, 'viewmode': viewmode, 'stype': stype, 'tags': tags, 'timestamp': timeStamp, 'span': timeSpan, 'limit': limit },
			async: true,
			context: this,
			crossDomain: true,
			success: function (res) {
