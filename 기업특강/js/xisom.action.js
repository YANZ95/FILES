...
(function (global) {
	...
	function handleSlide(event, view, planX, planY, state) {
		//console.log(event); console.log(view); console.log(plan);
	...
	function handleValueClickInstant(event, view, plan, state) {
		...
		view.addEventListener("mouseup", function _h(event) {
			setTimeout(function () { state.mouseup(event) }, plan.instantDelay);
			view.removeEventListener("mouseup", _h, false);
		}, false);
	...
	function handleValueClickInstantByTouch(event, view, plan, state) {
		...
		view.addEventListener("touchend", function _h(event) {
			setTimeout(function () { state.touchend(event) }, plan.instantDelay);
			view.removeEventListener("touchend", _h, false);
		}, false);
	...
	////////////////////////////////////////////////////////////
	// ColorHover(색상 호퍼 설정) 등록
	////////////////////////////////////////////////////////////
	function registerHoverClick(view, plan) {
		var state = {
			fillStyle: null
			, strokeStyle: null
			, textStyle: null
		...
		view.addEventListener("mouseenter", function (e) {
			handleColorHover(e, view, plan, state);
		}, false);
		view.addEventListener("mouseover", function (e) {
			page.onMouseOver(e, view);
		}, false);
	...
	// ColorHover(색상 설정) 처리
	////////////////////////////////////////////////////////////
	function handleColorHover(event, view, plan, state) {
		//console.log(event); console.log(view); console.log(plan);
		...
		try {
			if (view.fillStyle != null) {
				...
				state.background = view.background;
				if (state.background != null) {
					view.setBackgroundColor(plan.fillStyle);
				}
			}
		} catch (e) { }

		try {
			if (view.strokeStyle != null) {
				state.strokeStyle = view.strokeStyle;
				if (state.strokeStyle != null) {
					view.setStrokeStyle(plan.strokeStyle);
				}
				...
				state.foreground = view.foreground;
				if (state.foreground != null) {
					view.setForegroundColor(plan.strokeStyle);
				}
			}
		} catch (e) { }

		if (state.mouseleave == null) {
			state.mouseleave = function (event) {
				//console.log(event); console.log(view); console.log(plan);

				view.removeEventListener("mouseleave", state.mouseleave, false);
				page.events.releaseMouse();
				...
				if (state.fillStyle != null) {
					...
					view.setBackgroundColor(state.background);
				...
				if (state.strokeStyle != null) {
					...
					view.setForegroundColor(state.foreground);
					state.foreground = null;
				}
			};
		}
		view.addEventListener("mouseleave", state.mouseleave, false);
	...
	page.createActions = function (action) {
		...
		for (var j = 0; j < planCount; j++) {
			...
			} else if (plan.type == "ColorHover") {
				registerHoverClick(view, plan);
