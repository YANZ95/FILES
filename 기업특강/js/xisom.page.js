...
(function () {
	...
	PageManager.prototype.setZoom = function () {	   
		...
		if(bodyHeight * scale > htmlHeight + 2) {
	...
	if (typeof __pageOption !== "undefined" && __pageOption.zoom == true) {
...
(function () {
	...
	page.actions = new function () {
		this.pageClick = function (event, view, plan) {
			...
			if (plan.page.indexOf("http://") > -1 || plan.page.indexOf("https://") > -1 || plan.page.indexOf("www.") > -1) {
				...
				url = plan.page + ".html";
				if (plan.argument != null && plan.argument != "") {
					url += "?_pageArgs=" + plan.argument;
				}
			}
			var mode = plan.mode;
			if (mode == "Open") {
			...
			} else if(mode == "Frame" && plan.frame != ""){
				var frame = $XV(plan.frame);
				if(frame != null){
					frame.setURL(url);
				}
			}
	...
	page.keyboardOffset = function (id, obj, kbWidth, btnWidth) {
		...
		var target = $("#" + id).children();
		var targetX = Number(target.attr('x'));
		var targetY = Number(target.attr('y'));
		var targetH = Number(target.attr('height'));
		var targetW = Number(target.attr('width'));
		var contentW = $('.contents').width();
		var contentH = $('.contents').height();
		var kb = $(obj).data('keyboard');
		var isBottom = (contentH - targetY) < dHeight;
		var isRight = (contentW - targetX) < dWidth;
		var newX = contentW - (targetX + targetW);
		var newY = targetY + targetH;

		if (isBottom && !isRight) {
		    kb.position.my = 'left bottom';
		    kb.position.at = 'left+' + targetX + ' top+' + targetY;
		} else if (isRight && !isBottom) {
		    kb.position.my = 'right top';
		    kb.position.at = 'right-' + newX + ' top+' + newY;
		} else if (isBottom && isRight) {
		    kb.position.my = 'right bottom';
		    kb.position.at = 'right-' + newX + ' bottom-' + targetH;
		} else {
		    kb.position.my = 'left top';
		    kb.position.at = 'left+' + targetX + ' top+' + newY;
		}

		kb.position.orig_at = kb.position.at;
		kb.redraw();
	    //--------------------------------------------------------------//
	...
	page.valueToDateTimeFormat = function (value, format) {
		// NEW UPDATE => Using https://github.com/phstc/jquery-dateFormat
		if (!format) format = "yyyy-MM-dd";
		format = format.replace(/f/gi, 'S').replace('dddd', 'DDD').replace('ddd', 'E').replace('DDD', 'ddd').replace(/t/gi, '').trim();
		var localTime = moment.utc(value).toDate();
		return $.format.date(localTime, format);
    ...
    //Add Event Listener
    page.addEventListener = function (type, callback) {
        document.addEventListener(type, callback);
    };

    //Remove Event Listener
    page.removeEventListener = function (type, callback) {
